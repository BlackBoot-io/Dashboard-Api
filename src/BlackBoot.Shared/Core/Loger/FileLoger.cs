...
public class FileLoger
{
    ...
    public static void Info(string log, string path = "")
    {
        ...
        try
        {
            using var stream = File.AppendText($"{path}\\Info-{DateTime.Now.ToShortDateString().Replace("/", "-")}.txt");
    ...
    public static void Error(Exception e, string path = "")
    {
        ...
        try
        {
            using var stream = File.AppendText($"{path}\\Error-{DateTime.Now.ToShortDateString().Replace("/", "-")}.txt");
            stream.WriteLine(
